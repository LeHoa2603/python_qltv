...
cur.execute('DROP TABLE IF EXISTS quyen_thu_thu')
cur.execute('DROP TABLE IF EXISTS thu_thu_quyen')
...
# Tạo bảng Quyền hạn của người dùng
cur.execute('''
...
# Tạo bảng Quyền thủ thư
cur.execute('''
CREATE TABLE IF NOT EXISTS quyen_thu_thu (
    id_quyen INTEGER PRIMARY KEY AUTOINCREMENT,
    ten_quyen TEXT NOT NULL
)
''')

# Thêm các quyền cho thủ thư vào bảng quyen_thu_thu
quyen_thu_thu = [
    ('Thêm sách',),
    ('Quản lý độc giả',),
    ('Quản lý mượn trả',),
...
cur.executemany('INSERT INTO quyen_thu_thu (ten_quyen) VALUES (?)', quyen_thu_thu)

# Tạo bảng Liên kết thủ thư với quyền hạn
cur.execute('''
CREATE TABLE IF NOT EXISTS thu_thu_quyen (
    id_thu_thu INTEGER,
    id_quyen INTEGER,
    FOREIGN KEY (id_thu_thu) REFERENCES thu_thu (id_thu_thu),
    FOREIGN KEY (id_quyen) REFERENCES quyen_thu_thu (id_quyen),
    PRIMARY KEY (id_thu_thu, id_quyen)
)
''')
